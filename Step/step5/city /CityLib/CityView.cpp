...
void CityView::AddMenus(wxFrame* mainFrame, wxMenuBar *menuBar, wxMenu* fileMenu, wxMenu* viewMenu)
{
	...
	//
	// This Adds the Separator and to the Menu count
	//
    ...
    //
    // Append the menus to the menubar (Pop window)
    //
...
void CityView::OnLeftDoubleClick(wxMouseEvent &event)
{
	...
	// Break if clicked on nothing
	if (tile == nullptr)
	{
		return;
    }

	PadVisitor padVisitor(tile);
	mCity.Accept(&padVisitor);
	mGrabbedItem = nullptr;
	padVisitor.LuanchingStarShip();
	Refresh();
}
