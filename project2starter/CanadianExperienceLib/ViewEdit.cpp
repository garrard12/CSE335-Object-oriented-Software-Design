...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
	...
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED,&ViewEdit::OnSelectOne,this,XRCID("MachineOne"));
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED,&ViewEdit::OnSelectTwo,this,XRCID("MachineTwo"));
	//parent->Bind(wxEVT_COMMAND_MENU_SELECTED,&ViewEdit::OnSelectStatTime,this,XRCID("SetStartTime"));
...
/**
 * Handler for updating the machine one
 * @param event The event we update
 */
void ViewEdit::OnSelectOne(wxCommandEvent& event)
{
	GetPicture()->MachineDiaglog(this->GetParent(),1);
	Refresh();
}
/**
 * Handler for updating the machine two
 * @param event The event we update
 */
void ViewEdit::OnSelectTwo(wxCommandEvent& event)
{
	GetPicture()->MachineDiaglog(this->GetParent(),2);
	Refresh();
}

//void ViewEdit::OnSelectStatTime(wxCommandEvent& event)
//{
//
//}
