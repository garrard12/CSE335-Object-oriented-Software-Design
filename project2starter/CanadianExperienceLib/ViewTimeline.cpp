            ...
            wxBORDER_SIMPLE)
{
	...
	parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnStartTime, this, XRCID("SetStartTime"));
	//parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnEndTime, this, XRCID("EndTime"));
...
/**
 * Set the start time to a machine
 * @param event Menu event
 */
void ViewTimeline::OnStartTime(wxCommandEvent& event)
{
	StartTimeDlg startTimeDlg(this->GetParent(),GetPicture());
	startTimeDlg.ShowModal();
}
