...
std::shared_ptr<Machine> Machine2Factory::Create()
{
	...
	shared_ptr<Machine> machineTwo = make_shared<Machine>(mResourcesDir);
	...
	machineTwo->AddComponent(cymbal);
	...
	machineTwo->AddComponent(cymbalCylinder);
	...
	machineTwo->AddComponent(drumCylinder);
...
//	reader->GetSource(8)->SetSink(drumCylinder->GetSink());
	...
	for (int i = 0; i < 5; i++)
	{
		...
		machineTwo->AddComponent(tubing);
...
//	tubing->AddClamp(-180, -300, 0.5, 100);
	...
	machineTwo->AddComponent(clamps);
//
	//
	...
	machineTwo->AddComponent(backgroundRope);
